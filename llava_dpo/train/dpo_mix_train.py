...
from llava_dpo.train.llava_trainer_on_mix import DPOLLaVATrainer
#这里开始改了训练trainer
...
# 在 LazyContrastiveDataset 类中
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        image_tensor = None
        if has_image:
            raw_images = [source['image']] if 'image' in source else source['images']
            images = []
            ...
            if len(images) == 1:
                images = images + images # Duplicate for contrastive pairs
...
# --- 处理 MK 数据 ---
        ...
        print("vcd_sources_better:", vcd_sources_better)  #查看数据源
        print("vcd_sources_worse:", vcd_sources_worse)  #查看数据源
        print("mk_sources_better:", mk_sources_better)  #查看数据源
        print("sources_worse:", mk_sources_worse)  #查看数据源
